...
public class BotReport {
	...
	public void printHtmlOverview(PrintStream stream) throws IOException
	{
	...
	private  Html asList(Map<WSPathwayInfo, String[]> data2, boolean type)
			throws IOException{
		Html list = Html.ul();

		List<Map.Entry<WSPathwayInfo, String[]>> keys =
				new LinkedList<Map.Entry<WSPathwayInfo, String[]>>(data2.entrySet());

		Collections.sort( keys, new Comparator<Map.Entry<WSPathwayInfo, String[]>>(){
			public int compare( Map.Entry<WSPathwayInfo, String[]> o1,
								Map.Entry<WSPathwayInfo, String[]> o2 )
			{
				return compare(o1, o2);
			}
		} );
	      
		for (Entry<WSPathwayInfo, String[]> entry : keys){
			Html contents = Html.ul();
			String italic = null;
			String title = null;
			for (String result : entry.getValue()){
				String[] plit = result.split(",");
				for ( String sp : plit){
//					System.out.println(sp);
					if (type){
						...
						//						italic=result.getPwName();
						String href = "<a href="+entry.getKey().getUrl()
								+ " target='_null'>"+entry.getKey().getId()+"</a>";
						contents.addChild(
								Html.li(href+" (",
										Html.i(entry.getKey().getName()) + ") - ",
										Html.i(sp) ));
					}
				}
			}
			if (type){
				String href = "<a href="+entry.getKey().getUrl()
						+ " target='_null'>"+entry.getKey().getId()+"</a>";
				title = " - " ;
//				+species + " - " + entry.getValue().size()+ " identifier(s) outdated";
				list.addChild (Html.li (
						Html.b(href),title, Html.br(),
						Html.i(entry.getKey().getName()), Html.br(),
				...
				title = " - ";
				list.addChild (Html.li (
						Html.b(entry.getKey().getName()),title, Html.br(),						
